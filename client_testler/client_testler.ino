...
// T ID icin: kalici kimlik istiyorsan burayi degistirip yeniden yukle,
...
unsigned long ilkGonderimZamani = 0;     // İlk gönderim anı -> RTT için
...
// Serial monitor'den "T DROP <N>", "T CRC <N>", "T DELAY <ms>",
// "T STATUS", "T OFF" komutlari ile kontrol edilir.
uint16_t test_drop_count    = 0;   // Sonraki kac DATA atlanacak
uint16_t test_corrupt_count = 0;   // Sonraki kac DATA bozuk CRC ile gidecek
uint32_t test_extra_delay   = 0;   // Her gonderime eklenecek gecikme (ms)
...
void hamGonder(const String& mesaj) {
  ...
  // TEST: belirli sayida DATA gonderimini atla (kayip simulasyonu)
  ...
  // TEST: CRC'yi boz (sadece DATA icin)
...
// Test komutlarini Serial'dan oku (loop basinda cagrilir)
void testSerialOku() {
...
bool guvenliMesajGonder(const String& type, const String& payload) {
  ...
  durum = ACK_BEKLIYOR;
  denemeSayisi = 1;
  ilkGonderimZamani = millis();  // RTT için
...
// ====== Ana döngü ======
void loop() {
  testSerialOku();  // TEST: hata enjeksiyon komutlarini Serial'dan oku
